...
describe("ClientResourceHandler", () => {
  beforeEach(() => {
    getClientsSpy = spyOn(datevConnectClient.accounting, "getClients").mockResolvedValue([
      {
        id: "client-1",
        name: "Test Client 1",
    ...
    getClientSpy = spyOn(datevConnectClient.accounting, "getClient").mockResolvedValue({
      id: "client-123",
      name: "Test Client",
      consultant_number: "1001",
      client_number: "10001"
  ...
  describe("get operation", () => {
    ...
    test("handles parameters with default values for get", async () => {
      ...
      expect(getClientSpy).toHaveBeenCalledWith(context, "client-123", expect.objectContaining({
        host: "https://api.example.com",
        token: "test-token",
        clientInstanceId: "instance-1"
      }));
  ...
  describe("error handling", () => {
    test("throws NodeApiError for unsupported operation", async () => {
      ...
      ).rejects.toThrow('The operation "unsupportedOperation" is not supported for resource "client".');
    });
  ...
  describe("parameter handling", () => {
    ...
    test("correctly retrieves clientId parameter", async () => {
      ...
      expect(getClientSpy).toHaveBeenCalledWith(context, "test-client-id", expect.any(Object));
    });
  });
});