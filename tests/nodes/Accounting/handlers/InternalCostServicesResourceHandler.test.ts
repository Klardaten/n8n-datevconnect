...
describe("InternalCostServicesResourceHandler", () => {
  beforeEach(() => {
    createInternalCostServiceSpy = spyOn(datevConnectClient.accounting, "createInternalCostService").mockResolvedValue(mockCreatedInternalCostService);
  });
  ...
  describe("create operation", () => {
    ...
    test("creates complex internal cost service with allocation details", async () => {
      createInternalCostServiceSpy.mockResolvedValueOnce(mockComplexInternalCostService);
      const context = createMockContext({
        parameters: {
          costSystemId: "CS01",
          internalCostServiceData: JSON.stringify({
    ...
    test("creates internal cost service with minimal required data", async () => {
      ...
      expect(returnData[0].json.total_cost).toBe(1000.00);
    });
    ...
    test("creates billable internal cost service", async () => {
      ...
      expect(returnData[0].json.billable_rate).toBe(150.00);
  ...
  describe("error handling", () => {
    ...
    test("handles API errors gracefully when continueOnFail is true", async () => {
      createInternalCostServiceSpy.mockRejectedValueOnce(new Error("API Error: Validation failed"));
      const context = createMockContext({
        context: {
      ...
      expect(returnData).toHaveLength(1);
      expect(returnData[0].json).toEqual({ error: "API Error: Validation failed" });
    });
    ...
    test("handles network timeout errors", async () => {
      createInternalCostServiceSpy.mockRejectedValueOnce(new Error("Network timeout"));
      const context = createMockContext({
        context: {
          continueOnFail: mock(() => true)
        }
    ...
    test("handles authentication errors", async () => {
      createInternalCostServiceSpy.mockRejectedValueOnce(new Error("Unauthorized"));
      const context = createMockContext();
      const handler = new InternalCostServicesResourceHandler(context, 0);
      const returnData: any[] = [];
    ...
    test("handles validation errors from API", async () => {
      createInternalCostServiceSpy.mockRejectedValueOnce(new Error("Validation Error: Invalid cost center"));
      const context = createMockContext();
      const handler = new InternalCostServicesResourceHandler(context, 0);
      const returnData: any[] = [];
  ...
  describe("inheritance from BaseResourceHandler", () => {
    ...
    test("respects item index in error handling", async () => {
      createInternalCostServiceSpy.mockRejectedValueOnce(new Error("Test error"));
      const context = createMockContext({
        context: {
          continueOnFail: mock(() => true)
        }
  ...
  describe("parameter handling", () => {
    ...
    test("handles string representation of internalCostServiceData", async () => {
      const stringData = '{"service_name":"String Service","total_cost":1000.00}';
      const context = createMockContext({
        parameters: {
          costSystemId: "CS01",
  ...
  describe("data validation", () => {
    ...
    test("handles special characters in service names and descriptions", async () => {
      const serviceWithSpecialChars = {
        ...
        service_description: "Description with symbols: @#$%^&*()_+-={}[]|\\:;\"'<>?,./",
        department_code: "R&D",
        created_date: "2023-11-01T12:00:00Z",
        status: "active"
      };

      createInternalCostServiceSpy.mockResolvedValueOnce(serviceWithSpecialChars);
      const context = createMockContext({
        parameters: {
          costSystemId: "CS01",
          internalCostServiceData: JSON.stringify({
      ...
      expect(returnData[0].json.service_name).toBe("Service with Special Chars: & < > \" ' %");
      expect(returnData[0].json.service_description).toBe("Description with symbols: @#$%^&*()_+-={}[]|\\:;\"'<>?,./");
