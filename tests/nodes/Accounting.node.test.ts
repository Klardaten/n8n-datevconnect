...
describe("Accounting node", () => {
  test("node description is properly configured", () => {
    ...
    expect(accountingNode.description.displayName).toBe("Klardaten DATEVconnect: Accounting");
    ...
    const resourceProperty = accountingNode.description.properties?.find(p => p.name === "resource");
  ...
  test("has correct credentials configuration", () => {
    ...
    expect(accountingNode.description.credentials?.[0].name).toBe("datevConnectApi");
  ...
  test("has proper structure like MasterData node", () => {
    ...
    expect(accountingNode.description.description).toBe("Interact with DATEV Accounting API");
  ...
  test("resource operations are properly configured", () => {
    ...
    const clientOperations = properties.find(p => 
      p.name === "operation" && 
    ...
    const fiscalYearOperations = properties.find(p => 
      p.name === "operation" && 
    ...
    const accountsReceivableOperations = properties.find(p => 
      p.name === "operation" && 
      (p as any).displayOptions?.show?.resource?.includes("accountsReceivable")
    );
  ...
  test("ID parameters are properly configured", () => {
    ...
    const fiscalYearIdParams = properties.filter(p => p.name === "fiscalYearId");
    expect(fiscalYearIdParams.length).toBeGreaterThan(0);
    
    // Accounts Receivable ID parameter
    const accountsReceivableIdParam = properties.find(p => p.name === "accountsReceivableId");
    expect(accountsReceivableIdParam).toBeDefined();
    
    // Account Posting ID parameter
    const accountPostingIdParam = properties.find(p => p.name === "accountPostingId");
    expect(accountPostingIdParam).toBeDefined();
    
    // Accounting Sequence ID parameter
    const accountingSequenceIdParam = properties.find(p => p.name === "accountingSequenceId");
